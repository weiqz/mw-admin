...
<script>
import { getUserList, delUser, changeUserActive } from '../../api/user'
import { getdivimange } from '../../api/divimange'
    export default {
        ...
        computed: {
            ...
            demID(){
                return this.$route.params.depa_id;
            }
        },
        watch:{
            demID: function (){
        ...
        methods:{
            setdepartm(){
                this.formInline.dement = this.demID;
            },
            getUser(){
                getUserList({
            ...
            isvalidFn(row){
                this.$confirm('确定更改用户状态?', '提示', {
                    ...
                    changeUserActive({uid:row.id}).then((res)=>{
                        if(res.code==='0'){
